...
export async function GET(request: NextRequest) {
  try {
    ...
    if (search) {
      pacientes = await getPrismaClient().paciente.findMany({
        where: {
          nombre: {
            contains: search,
            mode: 'insensitive'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pacienteExistente = await getPrismaClient().paciente.findFirst({
    ...
    const paciente = await getPrismaClient().paciente.create({
