...
export async function POST(request: NextRequest) {
  try {
    ...
    let paciente = await getPrismaClient().paciente.findFirst({
      where: { nombre: nombre.trim() }
    });

    if (!paciente) {
      paciente = await getPrismaClient().paciente.create({
        data: { nombre: nombre.trim() }
      });
    }

    // Obtener última evaluación para comparación
    const ultimaEvaluacion = await getPrismaClient().evaluacion.findFirst({
    ...
    const evaluacion = await getPrismaClient().evaluacion.create({
      data: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (pacienteId) {
      evaluaciones = await getPrismaClient().evaluacion.findMany({
        where: { pacienteId: parseInt(pacienteId) },
        include: { paciente: true },
        orderBy: { fechaHora: 'desc' }
      });
    } else if (nombre) {
      const paciente = await getPrismaClient().paciente.findFirst({
        where: { nombre: nombre.trim() }
      });
